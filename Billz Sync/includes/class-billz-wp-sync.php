...
class Billz_Wp_Sync {
	...
	private function load_dependencies() {
		...
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-billz-wp-settigns-admin.php';
	...
	private function define_admin_hooks() {
		...
		$plugin_setting = new Billz_Wp_Sync_Admin_Setting( $this->get_plugin_name(), $this->get_version() );
	...
	private function define_sync_runner_hooks() {
		...
		$this->loader->add_action( 'init', $plugin_sync_runner, 'billz_collect_products_action_init' );

		add_filter('plugin_action_links', 'add_billz_plugin_action_link', 10, 2);
		add_action( 'billz_collect_products', 'synchronize_products_from_billz_api' );
		add_action( 'billz_force_collect_products', 'synchronize_products_from_billz_api' );
		...
		$this->loader->add_action( 'woocommerce_after_order_object_save', $plugin_sync_transactions, 'save_post_shop_order' );
